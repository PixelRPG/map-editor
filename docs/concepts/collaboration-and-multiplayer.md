...
**Project metadata** (name / author / version / description / `defaultTileSize`) edited in the Data view rides `__project/meta.update`: the whole `name` + `properties` bag in one op, replaced wholesale on apply (same coarse-upsert contract as `entity.upsert` — `defaultTileSize` makes this non-cosmetic). Sent by `DataController.setProjectField` → `ProjectStore.commitProjectMeta`; the receiving `ProjectStore.applyRemoteProjectOp` applies via the engine's `applyProjectMetaUpdate`, persists `game-project.json`, and re-hydrates the Data view via the store's `project-meta-changed` event.
