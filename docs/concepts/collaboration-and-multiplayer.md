...
Plumbing is maker-side: `CollabSession.sendProjectOp` (stamps peer id + seq, sends on the always-present op channel — works without an attached engine) and `CollabSession.onProjectOpReceived` → `ProjectStore.applyRemoteProjectOp` (mutates the peer's `GameProjectData`, persists, and emits a typed change event so every lens — Characters, Things, Graphics, Game — re-hydrates). The `ProjectStore` (`apps/maker-gjs/src/services/project-store.ts`) is the **single owner** of every project-level write: each local mutation flows mutate → persist → broadcast → notify through it, so "persisted but not broadcast" is structurally impossible; `CastController` / `ObjectsController` are thin lenses over it.

**Project metadata** (name / author / version / description / `defaultTileSize`) edited on the Game page rides `__project/meta.update`: the whole `name` + `properties` bag in one op, replaced wholesale on apply (same coarse-upsert contract as `entity.upsert` — `defaultTileSize` makes this non-cosmetic). Sent by `GameController.setProjectField` → `ProjectStore.commitProjectMeta`; the receiving `ProjectStore.applyRemoteProjectOp` applies via the engine's `applyProjectMetaUpdate`, persists `game-project.json`, and re-hydrates the Game page via the store's `project-meta-changed` event.

**Per-map editor data** (today: the atlas card position `editorData.atlasX/atlasY`) rides `__project/map.editor-data` — atlas drags happen in the atlas view with **no live scene**, so a `Command` can't represent them. The op is keyed by the map's stable `MapData.id` and carries a *partial* `MapEditorData` patch, shallow-merged on apply (`applyMapEditorData`) so future editor-data keys reuse the same kind. Sent from the maker's atlas-drag persist path; on the receiving side `ProjectStore.applyRemoteProjectOp` patches the in-memory `MapData` and hands the map id to the host via the store's `map-editor-data-changed` event — the window (the owner of map-file IO) persists the map file and repositions the atlas card live.

**Sprite-set import** carries image bytes, so it can't be one op (a single SCTP send >64 KiB is silently dropped). It rides a chunked `__project/spriteset.add.chunk` transfer (`chunkSpriteSetAdd` → `SpriteSetAddReassembler`, same 16 KiB chunking as the snapshot path), surfaced via `CollabSession.sendSpriteSetAdd` / `onSpriteSetAddReceived` → `ProjectStore.applyRemoteSpriteSetAdd` (writes the PNG + descriptor into the peer's `spritesets/`, registers the set under the *same id* so referencing characters resolve). Because the import broadcasts before the character that uses it (reliable + ordered channel), the peer has the set registered by the time the character upsert lands — no empty-preview window. **Sprite-set delete** is the inverse: a tiny `__project/spriteset.remove` (id only) via `sendProjectOp`; `applyRemoteProjectOp` drops the reference, deletes the local `<id>.png` + `<id>.json`, and emits `sprite-sets-changed` so every page that surfaces sprite-sets re-hydrates — Library › Characters (character previews) and Library › Graphics (tileset + appearance galleries). **Sprite-set descriptor edits** (rename, animation add/edit/delete) broadcast as a chunked `__project/spriteset.update.chunk` op (`CollabSession.sendSpriteSetUpdate` → `applyRemoteSpriteSetUpdate`) — same chunking, tagged as an update so the receiver replaces the existing descriptor. **Per-tile property edits** (the Solid switch / surface on `SpriteData.tileProperties`) ride the same descriptor channel: `TilesController` delegates to `ProjectStore.setTileSolid`/`setTileSurface`, which persist + broadcast the update; receivers refresh live collision via `Engine.refreshTileSolidsForSpriteSet`. The project store owns the sprite-set CRUD + broadcast for every surface; the Graphics page routes its tileset create/delete to the store and its appearance/animation edits through the cast controller's public methods (which write via the store).

**Sprite-set descriptor update** — a rename, an animation edit, or a tile-property change (`sprites[].solid` / `tileProperties.surface`) — rides a chunked `__project/spriteset.update.chunk` transfer (`chunkSpriteSetUpdate` → `ChunkReassembler`; a fat tileset descriptor can exceed the SCTP ceiling too). It carries the **descriptor only, no image bytes**, surfaced via `CollabSession.sendSpriteSetUpdate` / `onSpriteSetUpdateReceived` → `ProjectStore.applyRemoteSpriteSetUpdate`, which merges via the engine's `applySpriteSetUpdate` (the peer's descriptor lands wholesale, but the local `image` reference is pinned so a peer can't repoint our PNG), persists, refreshes the galleries (`sprite-sets-changed`), and — when a scene is open — refreshes live tile collision via the store's `tile-properties-changed` event → `Engine.refreshTileSolidsForSpriteSet` so a peer's Solid edit applies to the open map immediately. On the send side every descriptor mutation flows through `ProjectStore` (`renameSpriteSet`, `mutateSpriteSetData` for animations, `setTileSolid` / `setTileSurface` — the Graphics page's Solid/Surface switches delegate here via `TilesController`), one write+broadcast path per descriptor.
...
- **Project-op wiring (maker)** — `apps/maker-gjs/src/services/collab-session.ts` (`sendProjectOp` / `onProjectOpReceived` for entities; `sendSpriteSetAdd` / `onSpriteSetAddReceived` + `sendSpriteSetUpdate` / `onSpriteSetUpdateReceived` + reassemblers for chunked sprite-set transfers) ↔ `cast-controller.ts` (`applyRemoteProjectOp` / `applyRemoteSpriteSetAdd` / `applyRemoteSpriteSetUpdate` + per-mutation broadcast; `tiles-controller.ts` delegates Solid/Surface edits here), wired in `services/session-coordinator.ts`.
- **App orchestration** — `apps/maker-gjs/src/services/`: `session-service.ts` (lifecycle state machine) with `session-state.ts` (pure state transitions), `session-discovery-index.ts` and `session-open-flow.ts`; `collab-session.ts` (per-session wiring: cursor + selection broadcast) with `collab-inbound-route.ts` (inbound demux), `collab-op-stamper.ts` (peer/seq stamping) and `collab-peer-colour.ts` (`colourForPeer`); `lan-discovery.ts` / `lan-signalling.ts` / `relay-signalling.ts`. `session-coordinator.ts` owns the session lifetime from the window's side.
- **AI collaborator** — `Engine` assistant API + `packages/gjs/src/widgets/editor/roster-chip.ts` (`RosterChip`, the presence + follow surface in the scene editor's context pill), driven from `apps/maker-gjs/src/services/collab-presence-controller.ts`; see [`ai-collaborator.md`](ai-collaborator.md).
