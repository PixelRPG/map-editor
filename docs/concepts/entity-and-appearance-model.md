...
- **Objects view** — the **general** lens (landed C6): a master-detail list over EVERY `entityLibrary` entry (world objects AND `character`-template cast members, the latter flagged with a "Cast" badge), edited raw through the generated component inspector. Also the placement source: the scene-editor object brush lists every entity except the player actor (so Cast NPCs are placeable; the player spawns at its spawn-point).
- **Cast view** — the **specialised** friendly lens over the character subset of the same library: name, Appearance **picker**, speed, player toggle; components/states/scripts behind the "all components" disclosure; an **"Edit appearance →"** deep-link into the Sheets animation editor. The sheet gallery + animation editor moved out to the Sheets view (C5). Cast + Objects edits cross-refresh (`onEntityLibraryChanged`) since they overlap.
