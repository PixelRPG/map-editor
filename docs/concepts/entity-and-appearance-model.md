...
| C2 | **Objects library view + templates** — `Objects` editor mode + master-detail view (`objects-view`), `ObjectsController` (CRUD on the non-character `entityLibrary`, `entity.upsert`/`entity.remove` collab), `entity-templates.ts` (NPC / item / teleport / event / spawn-point / blank), `win.new-object` / `win.open-object` actions, MCP `set_view objects`. | **landed** |
...
| C3b | **Object placement tool (UI)** — the `object` editor tool + canvas-click brush (places the selected library entity), wiring the commands. *Needs interactive GUI validation.* | **planned** |
| C4 | **Cast view rebased onto templates** — new-character seeds from the `character` template; "all components" disclosure via `EntityComponentsEditor`. | **planned** |
