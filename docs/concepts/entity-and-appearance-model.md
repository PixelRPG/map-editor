...
The tier itself is a fact about the person at the keyboard, not about the project: the maker keeps it in GSettings (`ui-tier`, default `simple`, sibling of `theme`), mirrors it as the stateful `app.full-view` action, and never puts it on the wire — a collaborator's tier is their own.
