...
- **Collab handshake blocked: host Excalibur engine doesn't init on a non-foreground instance** — surfaced by the new collab tooling. Two instances launch + address fine; host `start_session` → `hosting`, joiner discovers over LAN + connects to the signalling server, but the **host never sends the SDP offer**: `SessionService.openSession('host')` finds `engineProvider() === null`. Root cause: a labelled/background instance's window isn't realized, so its `WebGLBridge` `Gtk.GLArea` never fires `onReady` → `engine.excalibur` stays `null` (no error logged; `getDebugStatus.enginePresent` only checks the *widget*). So a maker can't host unless its window is actually shown. Options: make the engine initialise Excalibur without a visible GLArea (offscreen/Canvas2D-fallback for headless host), or have the orchestrator force the host window to map/foreground. Related to the documented WebGL `FillContainer`/GLArea realization quirks. *owner: engine / gjs, why: blocks agent-driven collab e2e; the v1 WebRTC pair-edit flow is in-flight*
