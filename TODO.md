...
- **MCP bridge / `org.pixelrpg.maker.Control` follow-ups** — the dev-debugging bridge (`apps/mcp-bridge` → `org.pixelrpg.maker.Control` + `org.gtk.Actions`) ships status / screenshot / action-driving + headless project load (`OpenProject` / `ListRecentProjects` / `ListTemplates`). Still wanted: (a) **absolute `set_zoom`** value (only stepped `zoom_in`/`zoom_out`/`zoom_reset` today); (b) **canvas-composite screenshot fallback** — `captureWidgetPng` relies on the GSK renderer compositing the WebGL `Gtk.GLArea`; confirmed working on the dev stack (NGL), but if another stack returns a blank canvas region, composite the Excalibur canvas PNG over the window texture; (c) **push notifications** — emit a `Control` D-Bus signal on state change (project opened, view switched) and forward as an MCP notification, so an agent learns of changes without polling `get_status`; (d) **extract a reusable `@gjsify/*` helper** (the `captureWidgetPng` + the generic Control interface) once it has a 2nd consumer (e.g. easy6502). *owner: maker / gjsify*
