...
- **MCP bridge / `org.pixelrpg.maker.Control` follow-ups** — the dev-debugging bridge (`apps/mcp-bridge` → `org.pixelrpg.maker.Control` + `org.gtk.Actions`) ships status / screenshot / action-driving. Still wanted: (a) **headless project load** — a `Control.OpenProject(path)` method so the agent can load a project without the file-picker (`open_scene` currently needs a project already open); (b) **absolute `set_zoom`** value (only stepped `zoom_in`/`zoom_out`/`zoom_reset` today); (c) **canvas-composite screenshot fallback** — `captureWidgetPng` relies on the GSK renderer compositing the WebGL `Gtk.GLArea`; if a stack returns a blank canvas region, composite the Excalibur canvas PNG over the window texture; (d) **extract a reusable `@gjsify/*` helper** (the `captureWidgetPng` + the generic Control interface) once it has a 2nd consumer (e.g. easy6502). *owner: maker / gjsify*
