...
- **MCP orchestrator / `org.pixelrpg.maker.Control` follow-ups** — `apps/mcp-bridge` is a dev-only orchestrator: status / screenshot / action-driving, headless project load (`OpenProject` / `ListRecentProjects` / `ListTemplates`), **multi-instance** (env `PIXELRPG_INSTANCE` → per-instance app-id/D-Bus name; `launch_instance` / `list_instances` / `stop_instance` + an `instance` selector on every tool), **collab session primitives** (`start_session` / `join_session` / `get_session_state`), **`paint_tile`** (operation-oriented → undo + collab op-sync; shared `tile-paint.service` with `TileEditorSystem`), **`set_zoom`**, **`present_window`** (+ `engineReady`/`mapped` in status). Decided against (re-add only if a real need shows up): canvas-composite screenshot fallback; D-Bus/MCP push notifications (an MCP agent acts on its next turn anyway — `get_status` polling covers it). Still wanted: (a) **extract a reusable `@gjsify/*` helper** once a 2nd consumer appears (e.g. easy6502); (b) **data-driven component projection** — a generic `get_components` that serialises the session-singleton's ECS components (auto-tracks new ones) instead of the hand-curated `get_status`. *owner: maker / gjsify*
- **Collab e2e verified through engine-attach; last blocker = WebGL engine needs a *visible* window** — the new tooling drove the full handshake and it now WORKS up to the engine-attach: with `present_window` the host engine inits → `start_session` (hosting) → joiner discovers over LAN → WebRTC connects → **snapshot transfers** (`captureProjectSnapshot` now loads lazy maps on demand — fixed) → joiner writes the sandbox + loads the host's full project (`awaiting-engine`). The **only** remaining blocker for fully-automated op-sync: the joiner's `WebGLBridge` `Gtk.GLArea` only realises (→ `engine.excalibur` set → `engineReady`) when its window is genuinely rendering, and on Wayland a background app can't reliably raise itself (`present_window` is best-effort), so two same-machine instances can't both have live engines at once → the joiner can't attach its engine → stuck at `awaiting-engine`. Fix: **initialise Excalibur without a visible GLArea** (offscreen / Canvas2D-fallback for a headless peer) — the real enabler for headless multi-instance collab testing. Manual workaround: a human focuses the joiner window → its engine inits → attaches → `connected` → op-sync (`paint_tile`) flows. Related to the documented WebGL `FillContainer`/GLArea realization quirks. *owner: engine / gjs, why: blocks fully-automated agent-driven collab op-sync*
