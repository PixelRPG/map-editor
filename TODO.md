...
- **MCP orchestrator / `org.pixelrpg.maker.Control` follow-ups** — `apps/mcp-bridge` is now a dev-only orchestrator: status / screenshot / action-driving, headless project load (`OpenProject` / `ListRecentProjects` / `ListTemplates`), **multi-instance** (env `PIXELRPG_INSTANCE` → per-instance app-id/D-Bus name; `launch_instance` / `list_instances` / `stop_instance` + an `instance` selector on every tool), and **collab session primitives** (`StartSession` / `JoinSession` / `GetSessionState` → `start_session` / `join_session` / `get_session_state`). Still wanted: (a) **absolute `set_zoom`**; (b) **canvas-composite screenshot fallback** (GSK GLArea capture works on the dev NGL stack; composite the Excalibur canvas PNG if a stack returns a blank region); (c) **push notifications** — emit a `Control` D-Bus signal on state change + forward as an MCP notification (no polling); (d) **extract a reusable `@gjsify/*` helper** once a 2nd consumer appears (e.g. easy6502); (e) a **paint/edit tool** (place a tile at a coord) so collab op-sync — not just session establishment — can be asserted. *owner: maker / gjsify*
