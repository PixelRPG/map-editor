...
[Workspaces] |root: ./ |packages/{engine,gjs} |apps/{maker-gjs,game-browser,mcp-bridge,signalling-server} |games/{zelda-like,blank-starter,minimalist-starter} |docs/

[Packages] |`engine`: Excalibur-based engine + editor logic (Resources, Components, Systems, MapFormat); platform-indep core, runs in GJS or browser |`gjs`: GTK4/libadwaita widgets that host the engine + Gdk-side preview pipeline (Sprite/SpriteSheet/ImageTexture for Gsk-snapshot rendering)

[Apps] |`maker-gjs`: the map editor (primary) |`game-browser`: browser-runtime template — seed for multi-platform game export |`mcp-bridge`: dev-only MCP↔D-Bus orchestrator for agent-driving the maker (`org.pixelrpg.maker.Control`) |`signalling-server`: stateless WebSocket relay for cross-network WebRTC signalling
...
[Game systems] A **game system** (`GameSystemSpec`, `packages/engine/src/game-systems/`) is the switchable bundle a USER turns on — components, editor templates, ECS systems — never an ECS `System`. Every `ComponentSpec` names its owner in `system:`, `BUILT_IN_COMPONENT_SPECS` is DERIVED from the specs barrel, and `game-systems/registry.spec.ts` fails the build unless every component spec is owned by exactly one registered system (plus: no stale registrations, `requires` closes without a cycle, no template seeding a component its system can't render). **A new component therefore needs a `system:` and an owner's `components` entry, or CI blocks it.** Every validator / spawn path / inspector takes `effectiveComponentRegistry(project)`, never the full map. Switching a system off is DORMANT (data preserved + inert, reported apart from errors), never destructive; an unknown component type stays a loud failure. The enabled set is project data and rides `__project/systems.set` through `ProjectStore.setGameSystemEnabled`. Full model: `docs/concepts/game-systems.md`.
...
|internal deps → workspace refs (`workspace:^` or `workspace:*`); `gjsify install` symlinks each child's `node_modules/@pixelrpg/*` → sibling workspace source |external deps → exact versions, hoisted into root `node_modules/` |script names per package: `build`, `check` (tsc), `build:barrels` / `check:barrels` (gjsify barrels), `start` (apps only), `test` (engine, gjs, maker-gjs, signalling-server, mcp-bridge — each `gjsify test` + a hand-maintained `src/test.mts`) |add deps: `cd <pkg-dir> && gjsify install <name>` (writes the spec into that workspace's package.json) |root: `gjsify install` rehoists + relinks; `gjsify install --immutable` for CI

## Build / format / lint / test (root-level scripts)

|`gjsify foreach build -v -t` — topological build (root `build`) |`gjsify foreach check -v -t` — type-check + barrel regen across all packages |`gjsify foreach check:barrels -v -t` — drift guard (CI: any stale barrel exits non-zero) |`gjsify run check:specs` (root, `node scripts/check-spec-registration.mjs`) — spec-registration guard: fails if any `*.spec.ts` isn't imported + passed to `run()` in its package's hand-maintained `src/test.mts` (the silent-skip trap; CI gates it) |the OTHER eight repo guards run the same way — `check:assets`, `check:phantom-deps`, `check:layer-visibility`, `check:mode-routes`, `check:blp-actions`, `check:signals`, `check:orphan-components`, `check:chrome-stages` (`scripts/check-*.mjs`, one CI step each; TODO.md § Cleanup / debt says what bug class each one covers) |`gjsify foreach test -v -p --include @pixelrpg/engine --include @pixelrpg/gjs --include @pixelrpg/maker-gjs --include @pixelrpg/signalling-server --include @pixelrpg/mcp-bridge` — all test suites (`@gjsify/unit`; this is the root `test`; CI runs the same five suites as separate steps — use one `--include` or `gjsify workspace <pkg> test` for a single suite) |`gjsify workspace @pixelrpg/maker-gjs start` — run the editor (root `start`) |`gjsify run format` / `gjsify run lint` / `gjsify run check:format` — Biome direct (`biome.json`; the gjsify CLI's own lint/fix subcommands wrap oxlint/oxfmt, which this repo does not use). GTK CSS (`*.css`) is excluded from Biome — it is a GTK dialect Biome cannot parse; the gjsify CSS plugin owns it
