...
class Resource {
    ...
    constructor(resourceFilePath: string, readonly resourcePath: string, readonly fallbackDirPath: string) {
    ...
    get(path: string) {
        ...
        if(!path.startsWith(this.resourcePath)) {
            console.log("path not starting with resourcePath", path, this.resourcePath);
            path = this.resourcePath + path;
        }
        try {
    ...
    stream(path: string): Gio.InputStream | null {
        ...
        try {
            const stream = this.resource.open_stream(path, Gio.ResourceLookupFlags.NONE);
...
export const clientResource = new Resource('./org.pixelrpg.map-editor.data.gresource', CLIENT_RESOURCE_PATH, CLIENT_DIR_PATH.get_path()!);
