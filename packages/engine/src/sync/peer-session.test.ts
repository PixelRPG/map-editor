...
/**
 * Unit tests for {@link PeerSession} using mocked WebRTC + in-memory
 * signalling. Tests the negotiation handshake, channel wiring, and
 * the typed event surface — the real WebRTC stack only ships under
 * GJS (`@gjsify/webrtc`) and a browser, so an actual peer-to-peer
 * smoke is reserved for the manual maker walk-through.
 */
...
class FakeRTCPeerConnection {
  ...
  /** Trigger the joiner-side `ondatachannel` for a host-emitted channel. */
...
/**
 * Build a constructor-shaped factory whose `new` invocation returns
 * the pre-baked fake instance. Arrow functions can't be called with
 * `new`, hence the explicit `function`.
 */
...
describe('PeerSession', () => {
  it('host creates op + awareness channels with the documented reliability modes', () => {
    ...
    expect(fakePc.channels.map((c) => c.label).sort()).toEqual([CHANNEL_AWARENESS, CHANNEL_OP])
  })

  it('host emits SDP offer when connect() runs, forwards through signalling', async () => {
    ...
    // microtask flush so the in-memory transport forwards
    await new Promise((r) => queueMicrotask(() => r(undefined)))

    expect(fakePc.localDescription?.type).toBe('offer')
    expect(hostTransport.sent[0]).toMatchObject({ type: 'sdp', payload: { type: 'offer' } })
    expect(joinerTransport.sent.length).toBe(0)
  })

  it('joiner answers an incoming offer and forwards the SDP back', async () => {
    const { hostTransport, joinerTransport } = pair()
    const fakePc = new FakeRTCPeerConnection()
    const session = new PeerSession({
      role: 'joiner',
    ...
    // joiner replies with answer; one of the sent messages is the answer
    const answer = joinerTransport.sent.find((m) => m.type === 'sdp' && m.payload.type === 'answer')
    expect(answer).toBeDefined()
  })

  it('forwards ICE candidates over signalling and applies inbound ones', async () => {
    const { hostTransport, joinerTransport } = pair()
    const fakePc = new FakeRTCPeerConnection()
    const session = new PeerSession({
      role: 'host',
    ...
    expect(hostTransport.sent.at(-1)).toMatchObject({
      type: 'ice-candidate',
      payload: { candidate: 'a=fake' },
    })

    joinerTransport.send({ type: 'ice-candidate', payload: { candidate: 'a=remote' } })
    await new Promise((r) => queueMicrotask(() => r(undefined)))
    expect(fakePc.addedIce).toEqual([{ candidate: 'a=remote' }])
  })

  it('emits state-changed → connected once both channels open', () => {
    const { hostTransport } = pair()
    const fakePc = new FakeRTCPeerConnection()
    const session = new PeerSession({
      role: 'host',
  ...
  it('routes inbound channel messages to op-received vs awareness-received', () => {
    const { hostTransport } = pair()
    const fakePc = new FakeRTCPeerConnection()
    const session = new PeerSession({
      role: 'host',
    ...
    expect(ops).toEqual([{ kind: 'tile.paint', x: 1 }])
    expect(aware).toEqual([{ kind: 'cursor', x: 12 }])
  })

  it('sendOp emits error when the op channel is not open', () => {
    const { hostTransport } = pair()
    const fakePc = new FakeRTCPeerConnection()
    const session = new PeerSession({
      role: 'host',
  ...
  it('sendAwareness silently drops when the channel is not open', () => {
    const { hostTransport } = pair()
    const fakePc = new FakeRTCPeerConnection()
    const session = new PeerSession({
      role: 'host',
  ...
  it('close() sends bye + tears down idempotently', () => {
    const { hostTransport } = pair()
    const fakePc = new FakeRTCPeerConnection()
    const session = new PeerSession({
      role: 'host',
    ...
    expect(hostTransport.sent.find((m) => m.type === 'bye')).toMatchObject({
      type: 'bye',
      payload: { reason: 'test' },
    })
  ...
  it('joiner attaches to incoming channels via ondatachannel', () => {
    ...
    expect(fakePc.channels).toEqual([]) // joiner does not pre-create
