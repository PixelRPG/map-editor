 ...
 *
 * Hosts an `Adw.ViewStack` + `Adw.ViewSwitcherBar` with three pages:
 * **Tiles**, **Layers**, **Props**. Each tab is exposed as a property
 * so the application shell can populate it (tiles, stamps, layer list,
 * scene metadata).
 */
    ...
    GObject.registerClass(
      {
        ...
        Template,
        InternalChildren: ['stack', 'tiles_tab', 'layers_tab', 'props_tab'],
        Properties: {
          'visible-page': GObject.ParamSpec.string(
            'visible-page',
            'Visible Page',
            'Name of the active tab (tiles, layers, props)',
