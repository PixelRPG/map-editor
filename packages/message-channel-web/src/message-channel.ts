...
export class MessageChannel {
  ...
  constructor(
    ...
    if (this.context === IframeContext.PARENT && !this.targetIframe) {
      console.warn(
        'MessageChannel: targetIframe is required for parent context',
      )
    }
   ...
   */
  public postMessage(message: BaseMessage): void {
    // Add channel name to the message to scope it
    const scopedMessage: BaseMessage = {
      ...message,
    ...
    if (this.context === IframeContext.PARENT && this.targetIframe) {
      ...
      if (this.targetIframe.contentWindow) {
        ...
        console.error(
          'MessageChannel: Cannot send message, iframe contentWindow is not available',
        )
      }
      ...
      console.error(
        'MessageChannel: Cannot send message, invalid context or missing target',
      )
    }
  }
