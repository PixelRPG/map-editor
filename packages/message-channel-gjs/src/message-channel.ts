...
import {
  MessageChannel as BaseMessageChannel,
  MessageEvent,
...
export class MessageChannel extends BaseMessageChannel {
  ...
  constructor(
    channelName: string,
    protected readonly webView: WebKit.WebView,
  ) {
    super(channelName)
   ...
   * Unregisters the script message handler and disconnects signals.
   */
  close(): void {
    try {
      if (this.webView) {
        const userContentManager = this.webView.get_user_content_manager()
        // Unregister the script message handler
        userContentManager.unregister_script_message_handler(
          this.channelName,
          null,
        )
      }
