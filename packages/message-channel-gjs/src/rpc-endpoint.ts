...
> extends CoreRpcEndpoint<T> {
   ...
   */
  private static instances = new Map<string, RpcEndpoint<any>>()
   ...
   */
  public static getInstance<T extends RpcMethodRegistry>(
    channelName: string,
    ...
    return this.instances.get(instanceKey)! as RpcEndpoint<T>
  }
  ...
  private setupRpcMessageHandling(): void {
    ...
    //             const directReply = (response: WireRpcResponse) => {
    //                 try {
   ...
   */
  public override async sendNotification<K extends keyof T>(
    method: K,
