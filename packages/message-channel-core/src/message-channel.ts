...
function isChannelMessage(data: unknown): data is BaseMessage {
  return (
    typeof data === 'object' &&
    data !== null &&
    ('channel' in data || typeof (data as any).channel === 'string')
  )
}
...
export abstract class MessageChannel {
  ...
  protected handleMessageEvent(event: MessageEvent): void {
    // Check if the message has a valid channel property
    if (
      isChannelMessage(event.data) &&
      (!event.data.channel || event.data.channel === this.channelName)
    ) {
