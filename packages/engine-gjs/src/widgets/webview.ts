...
import { INTERNAL_PROTOCOL } from '@pixelrpg/data-gjs'
import Template from './webview.blp'
import { EventControllerInput } from '../utils/index.ts'
...
export class WebView extends WebKit.WebView {
  ...
  setResourcePaths(paths: string[]) {
    ...
    this.resourceManager = new ResourceManager(
      paths,
      this.resourceManager.gresourcePath,
    )
  ...
  protected initPageLoadListener() {
    const signalId = this.connect(
      'load-changed',
      (_source: this, loadEvent: WebKit.LoadEvent) => {
   ...
   */
  protected registerURIScheme(
    scheme: string,
    handler: (schemeRequest: WebKit.URISchemeRequest) => void,
  ) {
